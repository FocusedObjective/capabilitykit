...
function graphSvg(loaded: Awaited<ReturnType<typeof loadCapabilities>>, gapsById: Map<string, number>): string {
  ...
  const cx = width / 2;
  const cy = height / 2;
  const sorted = [...nodes].sort((a, b) => (dependentsById.get(b.id)?.length ?? 0) - (dependentsById.get(a.id)?.length ?? 0));
  const n = Math.max(sorted.length, 1);
  const positioned = sorted.map((node, i) => {
    const ring = 180 + Math.floor(i / 8) * 120;
  ...
  for (const item of positioned) {
    for (const dep of item.node.agent?.depends_on ?? []) {
      const source = byId.get(dep);
      if (!source) continue;
      edgeLines.push(`<line x1="${source.x.toFixed(1)}" y1="${source.y.toFixed(1)}" x2="${item.x.toFixed(1)}" y2="${item.y.toFixed(1)}" marker-end="url(#arrow)" />`);
    }
  }

  const nodeLines = positioned.map(({ node, x, y, r, impact }) => {
    const gaps = gapsById.get(node.id) ?? 0;
    const statusClass = gaps > 0 ? "gap" : node.status === "planned" ? "planned" : node.status === "in-progress" ? "review" : "implemented";
    const tooltip = `${node.id} • ${node.status} • impacts ${impact} direct dependents • verification gaps ${gaps}`;
    const label = node.title.length > 22 ? `${node.title.slice(0, 20)}…` : node.title;
    return `<g class="node ${statusClass}" transform="translate(${x.toFixed(1)} ${y.toFixed(1)})"><title>${escapeXml(tooltip)}</title><circle r="${r}" /><text y="6">${escapeXml(label)}</text></g>`;
  });

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc">
  <title id="title">Capability dependency graph</title>
  <desc id="desc">Generated by capabilitykit graph. Node size reflects direct downstream dependents.</desc>
  <defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="7" refY="5" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="#94a3b8" /></marker></defs>
  <style>
    svg { background: #f8fafc; font-family: Inter, ui-sans-serif, system-ui; }
    .edges line { stroke: #cbd5e1; stroke-width: 2; opacity: 0.9; }
    .node circle { fill: white; stroke-width: 4; }
    .node text { text-anchor: middle; font-size: 12px; font-weight: 700; fill: #0f172a; }
    .implemented circle { stroke: #10b981; }
    .review circle { stroke: #3b82f6; stroke-dasharray: 7 6; }
    .gap circle { stroke: #f43f5e; }
    .planned circle { stroke: #f59e0b; stroke-dasharray: 5 5; }
  </style>
